...
class Site_parser(object):
    ...
    def _get_produkt_info(self, url, cat_name):
        ...
        logger.debug(f"{produkt_name}  {articul}  {produkt_price} {short_prod_discr} {prod_discr} {prod_images} {'ПРОДУКЦИЯ, ТМ СВАРОГ, Сопутствующие товары, ' + str(cat_name)} ")
...
def start():
    ...
    for item in ul:
        ...
        if i>999999999999999999991:
                break


    logger.debug(all_produkts)
    write_items_in_file(all_produkts, 'rezalt/files_uploads_accessoriese.csv')
